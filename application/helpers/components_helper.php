...
function breadcrumb(array $breadcrumb = [], string $title = '')
{
    ...
    foreach ($breadcrumb as $k => $v) {
        if ($i == $count) {
            ...
            $html .= '<li class="breadcrumb-item text-sm"><a class="opacity-5 text-dark" style="text-decoration: none" href=" ' . ((isset($v['url'])) ? $v['url'] : 'javascript:void(0)') . '" ' . ((isset($v['action'])) ? 'onclick="' . $v['action'] . '"' : '') . '>' . $v['text'] . '</a></li>';
        }
        $i++;
    }
 ...
 * @return html
 */
function selectWithFormGroup(string $id = '', string $label = '', string $name = '', array $option = [], string $selected = '', array $class = [], array $attribute = [])
{
    $attribute["id"] = $id;
    $html = '
    <div class="form-group">
        ...
        '
        . select($name, $option, $selected, $class, $attribute) .
        '
        <p class="text-danger mt-1" ' . $name . 'Err></p>
    </div>
    ';
...
function table(string $id = '', $col = [], array $class = [])
{
    $stringClass = implode(' ', $class);
