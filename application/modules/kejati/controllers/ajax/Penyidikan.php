...
class Penyidikan extends MX_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model($this->module . '/penyidikan_model', 'penyidikan');
    ...
    public function data()
    {
        ...
        if (!in_array('RPENYIDIKAN', $userPermission)) {
            ...
            $data['breadcrumb'] = breadcrumb([
                [
                    "text" => "Kejati",
                    "url" => base_url('kejati/penyidikan')
                ],
                [
                    "text" => "Penyidikan",
                ]
    ...
    {
        $userPermission = getPermissionFromUser();
        $list = $this->penyidikan->get_datatables();
        $data = array();
        foreach ($list as $penyidikan) {
            $row = array();
            $row[] = '  <p class="text-sm d-flex py-auto my-auto"><b>' . $penyidikan->no_surat_tugas . '</b></p>';
        ...
        $output = array(
            "draw" => @$_POST['draw'],
            "recordsTotal" => $this->penyidikan->count_all(),
            "recordsFiltered" => $this->penyidikan->count_filtered(),
            "data" => $data,
        );
        ...
        if (!(count(array_intersect($userPermission, ['CPENYIDIKAN', 'UPENYIDIKAN'])) > 0)) {
            ...
            if ($this->input->post('id') == '' || $this->input->post('id') == NULL) {
                ...
                if ($pengaduan == NULL) {
                    ...
                    $data['data'] =  $this->load->view($this->module . '/penyidikan/pengaduan', $pengaduan, TRUE);
                    $this->output
                        ->set_content_type('application/json')
                        ->set_output(
                            json_encode($data)
        ...
        if (!(count(array_intersect($userPermission, ['CPENYIDIKAN', 'UPENYIDIKAN'])) > 0)) {
            ...
            if ($this->input->post('id') == '' || $this->input->post('id') == NULL) {
                ...
                if ($sop == NULL) {
                    ...
                    $data['data'] =  $this->load->view($this->module . '/penyidikan/sop', $params, TRUE);
                    $this->output
                        ->set_content_type('application/json')
                        ->set_output(
                            json_encode($data)
    ...
    public function getPegawai()
    {
        ...
        $data['status'] = TRUE;
        $data['data'] = $this->load->view($this->module . '/penyidikan/sop_pegawai', $params, TRUE);
        $this->output->set_content_type('application/json')->set_output(json_encode($data));
    }
    ...
    public function addPegawaiHTML()
    {
        ...
        if (!in_array('CPENYIDIKAN', $userPermission)) {
            ...
            $data['data'] = modal('add_sop_pegawai', $this->load->view($this->module . '/penyidikan/add_sop_pegawai', $params, TRUE));
            $this->output->set_content_type('application/json')->set_output(json_encode($data));
        }
    }
    ...
    public function getKelengkapan()
    {
        ...
        $data['status'] = TRUE;
        $data['data'] = $this->load->view($this->module . '/penyidikan/sop_kelengkapan', $params, TRUE);
        $this->output->set_content_type('application/json')->set_output(json_encode($data));
    }
    ...
    public function uploadKelengkapanHTML()
    {
        ...
        if (!in_array('CPENYIDIKAN', $userPermission)) {
            ...
            $data['data'] = modal('upload_kelengkapan', $this->load->view($this->module . '/penyidikan/add_sop_kelengkapan', $params, TRUE));
            $this->output->set_content_type('application/json')->set_output(json_encode($data));
        }
    }
